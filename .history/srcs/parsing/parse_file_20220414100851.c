...
static int check_file(int ac, char **av)
{
	...
	if (ac != 2)
	{
		write(2, "Error\nArgument invalid.\n", 24);
		return(1);
		//ecriture alternative plus courte
		//return (printf("Error\nArgument invalid.\n"), 1);
	}
...
int	check_params(t_file *file)
{
	...
	while (parsing.next)
	{
		...
		if (param_id_found()) // C || F || NO || SO || WE || EA
			file->param."id" e param dans la struct ded= given_param; //storiee a cela
		etc ...
	}
...
//	map is ok
	...
	//WIP pour la 3D et tourner le pt de vue
