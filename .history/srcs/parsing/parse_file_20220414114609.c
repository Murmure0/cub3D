...
int	check_params(t_file *file)
{
	...
	while (1) //tmp->next
	{
		...
		if (map_is_found(tmp->content[i])) // map given last, donc plus de params apres map
		{
			file->map = tmp; //On degage les chaines params de la liste, on ne garde que la map
			free() previous lists;
			break;
		}
		else if (param_id_found()) // C || F || NO || SO || WE || EA
