...
int param_id_found(char c)
{
	if (c == 'C' || c == 'F' || c == 'NO' || c == 'SO'
		|| c == 'EA' || c == 'WE')
		return (1);
	return (0);
}
...
int	check_params(t_file *file)
{
	...
	while (1) //tmp->next
	{
		...
		else if (param_id_found(tmp->content[i]))) // C || F || NO || SO || WE || EA
		{
			fct -> file->param."id" = given_param; //store param dans la struct dediee a cela
			tmp = tmp->next;
		}
		else
